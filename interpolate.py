...
from src.utils import fix_randomness
...
def get_training_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True):
    ...
    cifar10_training_dict = DictWrapper(cifar10_training)
    cifar10_training_loader = DataLoader(
        cifar10_training_dict, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)
...
def get_test_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True):
    ...
    cifar10_test_dict = DictWrapper(cifar10_test)
    cifar10_test_loader = DataLoader(
        cifar10_test_dict, shuffle=shuffle, num_workers=num_workers, batch_size=batch_size)
...
def build_parser():
    ...
    # problem 
    parser.add_argument('--problem', type=str, choices=['vanilla', 'subgroup'], required=True, help="Problem type to solve: 'vanilla' (ERM) or 'subgroup' (class aggregated interpolation).")
    parser.add_argument('--num_groups', type=int, default=10,
                        help="Number of groups for subgroup constraints (only for 'subgroup').")
    ...
    parser.add_argument('--primal_lr', type=float, default=1e-3,
                        help="Primal optimizer learning rate.")
    parser.add_argument('--primal_wd', type=float, default=0.0,
                        help="Primal weight decay (L2).")
    parser.add_argument('--primal_beta1', type=float, default=0.9,
                        help="Adam beta1 for primal optimizer.")
    parser.add_argument('--primal_beta2', type=float, default=0.999,
                        help="Adam beta2 for primal optimizer.")
    # sgd config
    parser.add_argument('--primal_sgd_momentum', action='store_true', default=False,
                        help="Use SGD with momentum for the primal optimizer.")
    parser.add_argument('--sgd_momentum', type=float, default=1e-3,
                        help="Momentum value for SGD (when --primal_sgd_momentum is set).")
    parser.add_argument('--cosine_schedule', action='store_true', default=False,
                        help="Use cosine annealing LR schedule for the primal optimizer.")
    
    # optimizers: dual
    parser.add_argument('--use_dual_sgd', action='store_true', default=False,
            help="Use SGD for the dual optimizer instead of Adam.")
    parser.add_argument('--dual_lr', type=float, default=1e-3,
                        help="Dual optimizer learning rate.")
    parser.add_argument('--dual_wd', type=float, default=0.0,
                        help="Dual weight decay (L2).")
    parser.add_argument('--dual_beta1', type=float, default=0.9,
                        help="Adam beta1 for dual optimizer.")
    parser.add_argument('--dual_beta2', type=float, default=0.999,
                        help="Adam beta2 for dual optimizer.")
    
    # logging
    parser.add_argument('--logging_interval', type=int, default=1,
                        help='Log metrics to wandb every N iterations.')
    parser.add_argument('--checkpoint_interval', type=int, default=5,
                        help='Save a checkpoint every N epochs.')
    parser.add_argument('--save_final', action='store_true', default=False,
                        help="Save final primal model at the end of training.")
    parser.add_argument('--save_first', action='store_true', default=False,
                        help="Save model after the first epoch.")
    ...
    parser.add_argument('--wandbgrp', type=str, default='inter10',
                        help='Wandb run group.')
    parser.add_argument('--namestr', type=str, default='',
                        help='Optional suffix for the run name.')
    ...
    parser.add_argument('--eval_batch_size', type=int, default=139,
